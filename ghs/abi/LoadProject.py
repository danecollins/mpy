...
#DOC web: http://localhost:8008/LoadProject?name=full_url_to_project.emp
#DOC wiki: LoadProject full_url_to_project.emp
...
def main():
    ...
    if project_name:
        local_name = urltools.get_file(project_name,'emp')
        urltools.html_message("Opening project: %s" % local_name)
        awrde.LoadProject(local_name)
        if (sim_after):
            awrde.Simulate()
    else:
